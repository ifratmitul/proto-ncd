...
parser.add_argument("--no_pretrained_weights", default=False, action="store_true", help="disable ImageNet pretrained weights (use for ImageNet dataset)")
...
class Pretrainer(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        # metrics
        self.accuracy = Accuracy(task="multiclass", num_classes=self.hparams.num_labeled_classes)
...
def main(args):
    ...
    encoder_path = os.path.join(args.checkpoint_dir, 
                                f"encoder-{args.arch}-{args.dataset}.pth")
    torch.save(model.model.encoder.state_dict(), encoder_path)
