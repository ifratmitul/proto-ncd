...
def get_datamodule(args, mode):
    if mode == "pretrain":
        if args.dataset == "ImageNet":
            ...
            return PretrainCUBDataModule(args)
        else:
        ...
        if args.dataset == "ImageNet":
            ...
            return DiscoverCUBDataModule(args)
        else:
...
# =============================================================================
# CUB-200-2011 Dataset (200 bird species, 11,788 images)
# =============================================================================

class CUBDataset(torch.utils.data.Dataset):
    """CUB-200-2011 Dataset wrapper using ImageFolder structure."""

    def __init__(self, root, train=True, transform=None, target_transform=None):
        """
        Args:
            root: Path to CUB_200_2011 folder containing 'images' subfolder
        ...
        with open(labels_file, 'r') as f:
            id_to_label = {int(line.split()[0]): int(line.split()[1]) - 1 for line in f}  # 0-indexed
...
class PretrainCUBDataModule(pl.LightningDataModule):
    """CUB DataModule for Stage 1: Pretraining on labeled classes."""

    def __init__(self, args):
        super().__init__()
        self.data_dir = args.data_dir  # Path to CUB_200_2011 folder
        self.batch_size = args.batch_size
        self.num_workers = args.num_workers
        self.num_labeled_classes = args.num_labeled_classes
        self.num_unlabeled_classes = args.num_unlabeled_classes
        self.transform_train = get_transforms("unsupervised", "CUB")
        self.transform_val = get_transforms("eval", "CUB")

    def prepare_data(self):
        # CUB dataset should already be downloaded and extracted
        pass
    ...
    def setup(self, stage=None):
        ...
        train_indices_lab = np.where(
            np.isin(self.train_dataset.targets, labeled_classes)
        )[0]
        self.train_dataset = torch.utils.data.Subset(self.train_dataset, train_indices_lab)
        ...
        val_indices_lab = np.where(
            np.isin(self.val_dataset.targets, labeled_classes)
        )[0]
        self.val_dataset = torch.utils.data.Subset(self.val_dataset, val_indices_lab)
...
class DiscoverCUBDataModule(pl.LightningDataModule):
    """CUB DataModule for Stage 3: Novel Class Discovery."""
    ...
    def setup(self, stage=None):
        ...
        # train dataset (all classes)
        self.train_dataset = CUBDataset(
            self.data_dir, train=True, transform=self.transform_train
        )

        # val datasets
        val_dataset_train = CUBDataset(
            self.data_dir, train=True, transform=self.transform_val
        )
        val_dataset_test = CUBDataset(
            self.data_dir, train=False, transform=self.transform_val
        )

        # unlabeled classes, train set
        val_indices_unlab_train = np.where(
            np.isin(val_dataset_train.targets, unlabeled_classes)
        )[0]
        val_subset_unlab_train = torch.utils.data.Subset(val_dataset_train, val_indices_unlab_train)

        # unlabeled classes, test set
        val_indices_unlab_test = np.where(
            np.isin(val_dataset_test.targets, unlabeled_classes)
        )[0]
        val_subset_unlab_test = torch.utils.data.Subset(val_dataset_test, val_indices_unlab_test)

        # labeled classes, test set
        val_indices_lab_test = np.where(
            np.isin(val_dataset_test.targets, labeled_classes)
        )[0]
        val_subset_lab_test = torch.utils.data.Subset(val_dataset_test, val_indices_lab_test)
